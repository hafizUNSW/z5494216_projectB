...
for f in ("AGENTS.md", "CLAUDE.md"):
    if (ROOT / f).exists():
        ...
        if "replace this placeholder" not in text:
            agent_edited.append(f)
check(bool(agent_edited), f"your own agent file ({', '.join(agent_edited) or 'none'})",
      "edit AGENTS.md or CLAUDE.md (your tool's file) with your own "
      "instructions - both are still the provided stub")
...
if part == "B":
    ...
    warn("nltk" not in app_text, "app does not import nltk",
         "streamlit_app.py references nltk - the deployed app must read "
         "precomputed results/, not run VADER (the free tier cannot)")
